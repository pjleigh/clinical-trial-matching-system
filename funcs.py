...
# gets patientnum and info given ID to print out to GUI
def findpatient(patientinfo, patientID):
    patientnum = 0
    for i in patientinfo.loc[:, 'PatientID']:
        if i == patientID:
            info = [x for x in patientinfo.loc[patientnum, :]]
...
def gettrialinfo(trials, trialnum):
    ...
    trialinfo.append(trials.loc[trialnum, 0])
    trialinfo.append(trials.loc[trialnum, 3])
    trialinfo.append(trials.loc[trialnum, 5])
    trialinfo.append(trials.loc[trialnum, 18])
    trialinfo.append(trials.loc[trialnum, 24])
    trialinfo.append(trials.loc[trialnum, 25])
    trialinfo.append(trials.loc[trialnum, 29])
    trialinfo.append(trials.loc[trialnum, 30])
    trialinfo.append(trials.loc[trialnum, 32])
    trialinfo.append(trials.loc[trialnum, 33])

    inclusioncriteria = trials.loc[trialnum, 34]
    exclusioncriteria = trials.loc[trialnum, 35]
    ...
    if len(inclusioncriteria.split("exclusion criteria")) > len([inclusioncriteria]):
        trialinfo.append(inclusioncriteria.split("exclusion criteria")[0])
        trialinfo.append(inclusioncriteria.split("exclusion criteria")[-1])

    # seperate criteria
    else:
        trialinfo.append(inclusioncriteria)
        trialinfo.append(exclusioncriteria)
...
def getmeanage(patientinfo, patientnumlist):
    ages = []
    for i in patientnumlist:
        ages.append(convertpatientyears(patientinfo, i))
...
# returns 1 if ICTRP trial can accept patientdiagnoses, 0 if not
...
# converts EMRBots patient PatientDateOfBirth to years
...
# checks if patient gender fits within ICTRP trial gender reguirements. Returns 1 if so, 0 if not
...
# "wrapper" function that makes all checks 
# returns 1 if patient is eligible, 0 if not
